$(function(){
	...
	$("body").on("click",".add-ds",function(){
		if(i<26){
			$(".add-con").append("<li class='edittable-bottom'>" +
				...
				"<lable>样品编号</lable><input id='sample_num' name='sample_num' type='text' class='sample' readonly='readonly' value='"+$('.ypbh').val()+"'/></li><li>" +
				"<lable>菌株原始编号</lable><input name='original_num' type='text'  class='num'/><span class='err'></span></li><li class='form-a'>" +
				"<lable>文献信息</lable>" +
				"<a class='xz-btn-a'>选择文件</a> <input type='text' name='' id='' value='' class='file-name-a'  readonly='readonly' placeholder='仅支持doc/docx格式文档'/>" +
				"<input onchange='getPhotoSizes(this)' name='word_addr' type='file'  class='file-info-a' accept='.doc,.docx'/></li><li class='form-c'>" +
				"<lable>文本信息</lable>" +
				"<a class='xz-btn-c'>选择文件</a> <input   type='text' name='' id='' value='' class='file-name-c'  readonly='readonly' placeholder='仅支持txt格式'/>" +
				"<input onchange='getTextSize(this)' name='txt_addr' type='file' class='file-info-c' accept='.txt'/></li><li class='form-b'>" +
				"<lable>图片信息</lable>" +
				"<a class='xz-btn-b'>选择文件</a> <input type='text' name='' id='' value='' class='file-name-b'  readonly='readonly' placeholder='仅支持jpg/png格式'/>" +
	...
	$(".add-dsxx").click(function(){
		$('.addds').append('<li class="edittable-middle">' +
			...
			'<lable>毒素种类</lable>' +
			'<select name="toxinId" id="toxin_id1" onblur="checktoxin_id()">' +
